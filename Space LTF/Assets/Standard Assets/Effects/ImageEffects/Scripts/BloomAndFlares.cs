...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class BloomAndFlares : PostEffectsBase
    {
        ...
        private static readonly int _offsets = Shader.PropertyToID("offsets");
        private static readonly int _threshold = Shader.PropertyToID("_threshold");
        private static readonly int _tintColor = Shader.PropertyToID("tintColor");
        private static readonly int _stretchWidth = Shader.PropertyToID("stretchWidth");
        private static readonly int _intensity = Shader.PropertyToID("_Intensity");
        private static readonly int _colorBuffer = Shader.PropertyToID("_ColorBuffer");
        private static readonly int _colorA = Shader.PropertyToID("colorA");
        private static readonly int _colorB = Shader.PropertyToID("colorB");
        private static readonly int _colorC = Shader.PropertyToID("colorC");
        private static readonly int _colorD = Shader.PropertyToID("colorD");
        private static readonly int _threshold1 = Shader.PropertyToID("threshold");
        private static readonly int _useSrcAlphaAsMask = Shader.PropertyToID("useSrcAlphaAsMask");
        private static readonly int _vignetteIntensity = Shader.PropertyToID("vignetteIntensity");
        #endregion
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            for (int iter = 0; iter < bloomBlurIterations; iter++)
            {
                ...
                separableBlurMaterial.SetVector(_offsets, new Vector4(0.0f, spreadForPass * oneOverBaseSize, 0.0f, 0.0f));
                ...
                separableBlurMaterial.SetVector(_offsets, new Vector4((spreadForPass / widthOverHeight) * oneOverBaseSize, 0.0f, 0.0f, 0.0f));
            ...
            if (lensflares)
            {
                ...
                else
                {
                    ...
                    hollywoodFlaresMaterial.SetVector(_tintColor, new Vector4(flareColorA.r, flareColorA.g, flareColorA.b, flareColorA.a) * flareColorA.a * lensflareIntensity);
                    ...
                    hollywoodFlaresMaterial.SetVector(_offsets, new Vector4((sepBlurSpread * 1.0f / widthOverHeight) * oneOverBaseSize, 0.0f, 0.0f, 0.0f));
                    hollywoodFlaresMaterial.SetFloat(_stretchWidth, hollyStretchWidth);
                    Graphics.Blit(thirdQuarterRezColor, secondQuarterRezColor, hollywoodFlaresMaterial, 1);
                    thirdQuarterRezColor.DiscardContents();

                    hollywoodFlaresMaterial.SetFloat(_stretchWidth, hollyStretchWidth * 2.0f);
                    Graphics.Blit(secondQuarterRezColor, thirdQuarterRezColor, hollywoodFlaresMaterial, 1);
                    secondQuarterRezColor.DiscardContents();

                    hollywoodFlaresMaterial.SetFloat(_stretchWidth, hollyStretchWidth * 4.0f);
                    Graphics.Blit(thirdQuarterRezColor, secondQuarterRezColor, hollywoodFlaresMaterial, 1);
                    thirdQuarterRezColor.DiscardContents();
                    ...
                    if (lensflareMode == (LensflareStyle34)1)
                    {
                        ...
                        {
                            separableBlurMaterial.SetVector(_offsets, new Vector4((hollyStretchWidth * 2.0f / widthOverHeight) * oneOverBaseSize, 0.0f, 0.0f, 0.0f));
                            Graphics.Blit(secondQuarterRezColor, thirdQuarterRezColor, separableBlurMaterial);
                            secondQuarterRezColor.DiscardContents();
        ...
        {
            lensFlareMaterial.SetVector(_colorA, new Vector4(flareColorA.r, flareColorA.g, flareColorA.b, flareColorA.a) * lensflareIntensity);
            lensFlareMaterial.SetVector(_colorB, new Vector4(flareColorB.r, flareColorB.g, flareColorB.b, flareColorB.a) * lensflareIntensity);
            lensFlareMaterial.SetVector(_colorC, new Vector4(flareColorC.r, flareColorC.g, flareColorC.b, flareColorC.a) * lensflareIntensity);
            lensFlareMaterial.SetVector(_colorD, new Vector4(flareColorD.r, flareColorD.g, flareColorD.b, flareColorD.a) * lensflareIntensity);
        ...
        {
            if (doHdr)
                brightPassFilterMaterial.SetVector(_threshold1, new Vector4(thresh, 1.0f, 0.0f, 0.0f));
            else
                brightPassFilterMaterial.SetVector(_threshold1, new Vector4(thresh, 1.0f / (1.0f - thresh), 0.0f, 0.0f));
            brightPassFilterMaterial.SetFloat(_useSrcAlphaAsMask, useAlphaAsMask);
        ...
        private void Vignette(float amount, RenderTexture from, RenderTexture to)
        {
            ...
            {
                vignetteMaterial.SetFloat(_vignetteIntensity, amount);
