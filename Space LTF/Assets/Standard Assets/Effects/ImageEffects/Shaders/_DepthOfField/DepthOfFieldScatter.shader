 Shader "Hidden/Dof/DepthOfFieldHdr" {
	...
	static const int SmallDiscKernelSamplesUsing = 4;			
	...
	static const int NumDiscSamplesUsing = 6;
	...
	float4 fragBlurInsaneMQ (v2f i) : SV_Target 
	{
		...
		for(int l=0; l < NumDiscSamplesUsing; l++)
		{
	...
	{	
		...
		float4 centerTap = tex2D(_MainTex, i.uv1.xy);
		half opticLensMask = tex2D(_ScopeMask, i.uv).r;
        	if (opticLensMask > 0)
        	    return centerTap;
	...
	{			
		...
		for(int l=0; l < SmallDiscKernelSamplesUsing; l++)
		{
