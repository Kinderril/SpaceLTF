...
public class ChangingCounter : MonoBehaviour
{
    ...
    public void Init(int value = 0, int TOTAL_CHANGE_TIME_MS = 1000, int FPS = 30,string prefix="")
    {
        _prefix = prefix;
...
//        label = GetComponent<Text>();
        label.text = _prefix + value.ToString();
        curValue = value;
    }
    ...
    private void UpdateToField()
    {
        ...
        if (ss.Length > 1)
        {
            label.text = _prefix + ss.TrimStart('0');
        }
        else
        {
            label.text = _prefix + ss;
        }
