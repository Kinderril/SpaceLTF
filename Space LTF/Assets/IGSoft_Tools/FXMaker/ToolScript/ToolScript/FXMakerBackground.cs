...
//				SaveBackgroundPrefab();
			...
			// Thumb Selected
			if (bCloneType && m_CurrentBackgroundInfo.GetClildThumbFilename(nWinIndex) != "" && GUI.Button(new Rect(FXMakerLayout.m_rectInnerMargin.x, 48+FXMakerLayout.m_fScrollButtonHeight+3, FXMakerLayout.GetFixedWindowWidth()-FXMakerLayout.m_rectInnerMargin.x*2, 25), FXMakerTooltip.GetHcToolBackground("CreateWeapon Thumb", settingObj.name)))
			{
